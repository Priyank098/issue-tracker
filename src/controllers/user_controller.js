...
const createIssue = async (req, res, next) => {
  ...
  try {
    ...
    if (req.body.assignedTo) {
      ...
      if (!(await issue.save())) throw new Error("Data not inserted");
      else {
  ...
  try {
    if (req.body.assignedTo) {
      ...
      if (issueData.status === Status.values[0]) {
        ...
        issueData.status === Status.values[1] ||
        issueData.status === Status.values[2]
      ) {
        if (issueData.assignedTo == req.body.assignedTo) {
          validatingFields(title, description, priority);
          update_issue(req, res, Issue, "", "", "");
        } else {
          const previousAssignUserData = await User.findById(
  ...
  try {
    ...
    if (issueData.status === Status.values[0]) {
      const updateIssue = await Issue.findByIdAndUpdate(
        _id,
        { assignedTo: assignedTo, status: Status.values[1] },
        {
          new: true,
          runValidators: true,
        }
      ...
      if (issueData.assignedTo == req.body.assignedTo) {
        throw new Error(
          "Issue Already Assigned to this person.Please try different User",
          {
            cause: { status: 404 },
          }
        );
...
const updateStatus = async (req, res, next) => {
  try {
    if (!Status.values.includes(req.body.status)) {
      throw new Error(
        "Choose only from them 'unAssigned','NotStarted','inProgress','Completed'",
        {
          cause: { status: 404 },
        }
      );
    ...
    if (req.body.status == Status.values[0]) {
      ...
      const updateIssue = await Issue.findByIdAndUpdate(
        req.body._id,
        { assignedTo: null, status: req.body.status },
        {
          new: true,
          runValidators: true,
        }
...
const statusFilterCount = async (req, res, next) => {
  ...
  try {
    ...
    allIssues.map((issue) => {
      ...
      if (issue.status == Status.values[0]) unAssignedCount += 1;
      else if (issue.status == Status.values[1]) assignedCount += 1;
      else if (issue.status == Status.values[2]) inProgressCount += 1;
      else if (issue.status == Status.values[3]) completedCount += 1;
    });
    res.status(201).json({
      success: true,
      data: {
...
const commentsCount = async (req, res, next) => {
  try {
    ...
    const commentData = await Comment.countDocuments({
...
const sortByDate = async (req, res, next) => {
  try {
    const sortByDate = await Issue.aggregate([{ $sort: { createdAt: -1 } }]);
...
const sortByUpdate = async (req, res, next) => {
  try {
    const sortByUpdate = await Issue.aggregate([{ $sort: { updatedAt: -1 } }]);
...
const verifyPayment = async (req, res, next) => {
  try {
    ...
    var expectedSignature = crypto
      .createHmac("sha256", "u6ZSwHfgm1pgdS6fzpwoxmAs")
